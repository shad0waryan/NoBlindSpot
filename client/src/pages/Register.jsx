...
const Register = () => {
  ...
  useEffect(() => { document.title = `Create account — ${APP_CONFIG.name}`; }, []);
  ...
  const pwStrength = (() => {
    ...
    if (!p) return null;
    if (p.length < 6) return { label: "Too short", color: "bg-red-500", pct: 20 };
    let score = 0;
    if (p.length >= 8) score++;
    if (/[A-Z]/.test(p)) score++;
    if (/[0-9]/.test(p)) score++;
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-md animate-slide-up">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 font-body text-sm">Start mapping your knowledge with {APP_CONFIG.name}</p>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPw(!showPw)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition-colors p-0.5" tabIndex={-1}>
                  {showPw ? (
              ...
              {pwStrength && (
                <div className="mt-2 space-y-1">
                  <div className="h-1 rounded-full bg-slate-800 overflow-hidden">
                    <div className={`h-full rounded-full transition-all duration-300 ${pwStrength.color}`} style={{ width: `${pwStrength.pct}%` }} />
                  </div>
                  <p className="text-[10px] text-slate-500">{pwStrength.label}</p>
                </div>
              )}
            </div>
