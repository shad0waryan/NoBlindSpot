...
const STATUS_COLORS = {
  known: {
    border: "border-emerald-500/75",
    bg: "bg-emerald-500/[0.05]",

    badge:
      "bg-emerald-100 border border-emerald-500 text-black dark:bg-emerald-500/20 dark:border-emerald-400 dark:text-white",
    active:
      "bg-emerald-200 text-black border-emerald-600 dark:bg-emerald-500/30 dark:text-white dark:border-emerald-400",
  },

  partial: {
    border: "border-amber-500/75",
    bg: "bg-amber-500/[0.05]",

    badge:
      "bg-amber-100 border border-amber-500 text-black dark:bg-amber-500/20 dark:border-amber-400 dark:text-white",
    active:
      "bg-amber-200 text-black border-amber-600 dark:bg-amber-500/30 dark:text-white dark:border-amber-400",
  },
  ...
  unknown: {
    ...
    badge:
      "bg-slate-200 border border-slate-500 text-black dark:bg-slate-700 dark:border-slate-500 dark:text-white",
          ...
          >
            ...
            <div className="flex gap-1">
              {keys.map((k) => (
                <kbd
                  key={k}
                  className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-[13px] text-slate-400 font-mono"
                >
                  {k}
                </kbd>
              ))}
...
const MapView = () => {
  ...
  const cycleStatus = useCallback((nodeId) => {
    setProgress((prev) => {
      ...
      return {
        ...prev,
        nodeStatuses: { ...prev.nodeStatuses, [nodeId]: NEXT_STATUS[cur] },
      };
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        toast("Failed to save progress", { type: "error" });
      } finally {
        setSaving(false);
      }
    }, 800);
  ...
  const submitQuiz = () => {
    ...
    toast(`${correct}/${quizData.length} correct — ${pct}%!`, {
      type: correct === quizData.length ? "success" : "info",
    });
  };
  ...
  const loadPath = async () => {
    ...
    try {
      ...
      toast("Failed to generate learning path", { type: "error" });
      setActivePanel(null);
    } finally {
  ...
  const openNoteEditor = (nodeId) => {
    setEditingNote(nodeId);
    setNoteText(progress?.nodeNotes?.[nodeId] || "");
  };
  ...
  const handleKeyDown = useCallback(
    (e) => {
      ...
      if (e.key === "1") setViewMode("list");
        ...
        if (e.key === "ArrowRight") {
          setFlashIdx((i) => Math.min(flashDeck.length - 1, i + 1));
          setFlashFlipped(false);
  ...
  const filteredNodes = useMemo(() => {
    ...
    if (statusFilter !== "all")
      r = r.filter(
        (n) => (progress?.nodeStatuses?.[n.id] || "unknown") === statusFilter,
      );
    if (search.trim()) {
  ...
  const depthAnalytics = useMemo(() => {
    ...
    nodes.forEach((n) => {
      ...
      if (!d[depth]) d[depth] = { total: 0, known: 0, partial: 0, unknown: 0 };
      d[depth].total++;
      d[depth][progress?.nodeStatuses?.[n.id] || "unknown"]++;
    });
    return d;
  }, [nodes, progress]);
  ...
  const treeMatchesSearch = useCallback(
    (node) => {
            ...
            >
              {hasChildren ? (
                <button
                  ...
                  className="mt-0.5 w-5 h-5 rounded flex items-center justify-center text-slate-400 hover:text-white hover:bg-surface-hover transition-all shrink-0"
                >
                  <svg
              ...
              <button
                ...
                title={`Click to cycle: ${status} → ${NEXT_STATUS[status]}`}
              >
                <span
                  className={`text-xs inline-block transition-transform duration-200 hover:scale-125 ${
                    status === "known"
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {hasNote && (
                  <button
                    onClick={() => openNoteEditor(node.id)}
                    title="Has note"
                ...
                <span
                  className={`text-[9px] px-2 py-0.5 rounded-full capitalize font-medium ${STATUS_COLORS[status].badge}`}
                >
                  {status}
                </span>
              </div>
  ...
  const quizScore =
    quizSubmitted && quizData
      ? {
          correct: quizData.filter((q, i) => quizAnswers[i] === q.correct)
            .length,
  ...
  return (
    ...
    "
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-xl sm:text-3xl mt-6 mb-2 font-display font-bold text-white capitalize">
              {topicMap.topic}
            </h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() =>
                setActivePanel(activePanel === "timer" ? null : "timer")
              }
              className={`flex items-center gap-1.5 text-xs font-mono px-2.5 py-1 rounded-lg border transition-all ${timerRunning ? "border-brand-500/50 text-brand-400 bg-brand-500/10 animate-pulse" : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
            >
              <svg
                className="w-3 h-3"
                fill="none"
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400">
              <span
                className={`w-1.5 h-1.5 rounded-full ${saving ? "bg-amber-400 animate-pulse" : "bg-emerald-500"}`}
              />
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
          ...
          <div className="flex gap-1.5 flex-wrap">
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg border text-xs transition-all duration-200 font-medium border-pink-500/30 text-pink-400 hover:bg-pink-500/10 hover:border-pink-400"
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg border text-xs transition-all duration-200 font-medium border-amber-500/30 text-amber-500 hover:bg-amber-500/10 hover:border-amber-400"
            >
              Stats
            </button>
            <button
              onClick={handleExport}
              className="px-2.5 py-1 rounded-lg border text-xs transition-all duration-200 font-medium border-slate-600 text-slate-400 hover:border-slate-400 hover:text-slate-400"
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg border text-xs transition-all duration-200 font-medium border-red-500/30 text-red-400 hover:bg-red-500/10 hover:border-red-400"
            >
              Delete
            </button>
          </div>
              ...
              <div className="flex gap-1 items-center flex-wrap">
                {STATUS_FILTERS.map((f) => (
                  <button
                    ...
                    className={`px-2 py-1 text-[13px] rounded-md border capitalize transition-all font-medium ${statusFilter === f ? "border-brand-500 text-white bg-brand-500/10" : "border-slate-700 text-slate-400 hover:border-slate-500"}`}
                  >
                    {f}
                    {f !== "all" &&
                      ` (${f === "known" ? stats.known : f === "partial" ? stats.partial : stats.unknown})`}
                  </button>
                ))}
                <span className="w-px h-4 bg-slate-700 mx-0.5 hidden sm:block" />
                <button
                  onClick={() => bulkSetStatus("known")}
                  className="px-2 py-1 text-[13px] rounded-md border border-emerald-500/30 text-emerald-300 hover:bg-emerald-500/10 transition font-medium"
                >
                  All Known
                </button>
                <button
                  onClick={() => bulkSetStatus("unknown")}
                  className="px-2 py-1 text-[13px] rounded-md border border-slate-700 text-slate-400 hover:bg-slate-700 transition font-medium"
                >
                  Reset
                </button>
              </div>
        ...
        activePanel !== "timer" && (
          <div className="mb-3 shrink-0 animate-scale-in">
            <div className="glass rounded-2xl p-4 sm:p-5 max-h-[45vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <h3 className="text-white font-display font-semibold text-sm capitalize">
                    {activePanel === "path" ? "Learning Path" : activePanel}
                  </h3>
                  {quizScore && activePanel === "quiz" && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${quizScore.correct === quizScore.total ? "bg-emerald-500/20 text-emerald-300" : quizScore.correct >= quizScore.total / 2 ? "bg-amber-500/20 text-amber-300" : "bg-red-500/20 text-red-300"}`}
                    >
                ...
                <button
                  ...
                  className="text-[13px] text-slate-400 hover:text-white border border-slate-700 px-2 py-0.5 rounded-lg transition"
                >
                  Esc
                </button>
              </div>
                ...
                (quizLoading ? (
                  ...
                  <div className="space-y-3">
                    {quizData.map((q, qi) => (
                      <div
                        key={qi}
                        className={`p-3 rounded-xl border transition-all ${quizSubmitted ? (quizAnswers[qi] === q.correct ? "border-emerald-500/30 bg-emerald-500/5" : "border-red-500/30 bg-red-500/5") : "border-slate-700 bg-slate-800/30"}`}
                      >
                    ...
                    {!quizSubmitted ? (
                      ...
                      >
                        Submit ({Object.keys(quizAnswers).length}/
                        {quizData.length})
                      </button>
                    ) : (
                ...
                (pathLoading ? (
                  ...
                  <div>
                    <p className="text-xs text-slate-400 mb-3">
                      {pathData.length} concepts to study, in recommended order
                    </p>
                    <ol className="space-y-2">
                      {pathData.map((item, i) => {
                        ...
                        const matchNode = nodes.find(
                          (n) =>
                            n.label.toLowerCase() === item.label.toLowerCase(),
                        );
                        const nodeStatus = matchNode
                          ? progress.nodeStatuses?.[matchNode.id] || "unknown"
                          : "unknown";
                        return (
                          <li
                            key={i}
                            className="flex gap-3 items-start p-2.5 rounded-xl border border-slate-700/40 bg-slate-800/20 hover:bg-slate-800/40 transition-colors group"
                          >
                            <span className="shrink-0 w-7 h-7 rounded-lg bg-gradient-to-br from-teal-500/20 to-teal-500/5 text-teal-400 text-xs font-bold flex items-center justify-center font-mono">
                              {i + 1}
                            </span>
                            <div className="flex-1 min-w-0">
                            ...
                            {matchNode && nodeStatus !== "known" && (
                              <button
                                onClick={() => {
                                  updateStatus(matchNode.id, "known");
                                  toast(`"${item.label}" marked as known`, {
                                    type: "success",
                                    duration: 1500,
                                  });
                                }}
                                className="shrink-0 opacity-0 group-hover:opac  ity-100 px-2 py-1 text-[10px] rounded-lg border border-emerald-500/30 text-emerald-300 hover:bg-emerald-500/10 transition-all font-medium"
                              >
                                ✓ Known
                              </button>
                            )}
                            {nodeStatus === "known" && (
                              <span className="shrink-0 text-[10px] text-emerald-300 font-medium">
                                ✓
                              </span>
                            )}
                          </li>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      ...
                      <div
                        key={s.label}
                        className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/50 text-center"
                      >
                        <div
                          className={`text-xl font-display font-bold ${s.cls}`}
                        >
                          {s.val}
                        </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-800/20 border border-slate-700/30">
                    ...
                    <div className="h-2 rounded-full bg-slate-800 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${stats.total > 0 ? Math.round(((stats.known + stats.partial * 0.5) / stats.total) * 100) : 0}%`,
                        }}
                      />
                    </div>
                  </div>

                  <h4 className="text-xs text-white font-display font-semibold uppercase tracking-wider">
                    Coverage by Depth
                  </h4>
                  {Object.entries(depthAnalytics)
                    .sort(([a], [b]) => a - b)
                    .map(([depth, d]) => {
                      const pct =
                        d.total > 0 ? Math.round((d.known / d.total) * 100) : 0;
                      return (
                        <div key={depth} className="flex items-center gap-3">
                          <span className="text-[13px] text-slate-400 w-14 shrink-0 font-mono">
                            Lvl {depth}
                          </span>
                          ...
                          <span className="text-[13px] text-slate-400 w-16 text-right font-mono">
                            {d.known}/{d.total}
                          </span>
                        </div>
                      );
        ...
        <div className="mb-3 shrink-0 animate-scale-in">
          <div className="glass rounded-2xl p-5 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-white font-display font-semibold text-sm">
                  Flashcards{" "}
                  <span className="text-slate-400 font-body font-normal">
                    ({flashIdx + 1}/{flashDeck.length})
                  </span>
                </h3>
            ...
            >
              <div
                ...
                style={{
                  ...
                  transform: flashFlipped ? "rotateY(180deg)" : "",
                }}
              >
                {/* Front */}
                <div
                  ...
                  style={{
                    ...
                    background:
                      "linear-gradient(135deg, var(--color-surface-card), var(--color-surface))",
                  }}
                >
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full mb-3 ${STATUS_COLORS[currentFlashcard.status].badge}`}
                  >
                ...
                >
                  ...
                  {currentFlashcard.note && (
                    <p className="text-xs text-amber-300/70 italic mt-3 text-center">
                      Note: {currentFlashcard.note}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="flex justify-center gap-1 mt-3">
              {flashDeck
                .slice(Math.max(0, flashIdx - 4), flashIdx + 5)
                .map((_, i) => {
            ...
            <div className="flex items-center justify-center gap-3 mt-4">
              <button
                onClick={() => {
                  setFlashIdx(Math.max(0, flashIdx - 1));
                  setFlashFlipped(false);
                }}
                disabled={flashIdx === 0}
              ...
              <button
                onClick={() => {
                  cycleStatus(currentFlashcard.id);
                  toast(`Marked as ${NEXT_STATUS[currentFlashcard.status]}`, {
                    type: "info",
                    duration: 1000,
                  });
                }}
                className="px-3 py-1.5 rounded-lg border border-brand-500/30 text-brand-400 text-xs font-medium hover:bg-brand-500/10 transition-all"
              >
                Cycle Status
              </button>
              <button
                onClick={() => {
                  setFlashIdx(Math.min(flashDeck.length - 1, flashIdx + 1));
                  setFlashFlipped(false);
                }}
                disabled={flashIdx >= flashDeck.length - 1}
                className="px-3 py-1.5 rounded-lg border border-slate-700 text-slate-400 text-xs font-medium hover:border-slate-500 disabled:opacity-30 transition-all flex items-center gap-1"
              >
                Next
                <svg
        ...
        <div className="mb-3 shrink-0 animate-scale-in">
          <div className="glass rounded-2xl p-5">
            ...
            <div className="flex flex-wrap gap-1.5 mb-4 justify-center">
              {TIMER_PRESETS.map((p) => (
                <button
                  ...
                  className={`px-3 py-1 text-[13px] rounded-lg border transition-all font-medium ${
                    timerPreset === p.id
              ...
              <div className="flex items-center justify-center gap-3 mb-4 animate-view-in">
                <div className="flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    value={customFocus}
                    onChange={(e) =>
                      setCustomFocus(Number(e.target.value) || 1)
                    }
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    value={customBreak}
                    onChange={(e) =>
                      setCustomBreak(Number(e.target.value) || 1)
                    }
                    onBlur={() => applyCustomTimer(customFocus, customBreak)}
                    className="w-14 text-center text-sm bg-surface border border-slate-700 rounded-lg py-1 text-white font-mono focus:border-brand-500 focus:outline-none transition-colors"
                  />
            ...
            <div className="text-center">
              ...
              <div
                className={`text-5xl font-mono font-bold mb-4 tabular-nums ${timerMode === "focus" ? "text-white" : "text-emerald-300"}`}
              >
                {formatTimer(timerSeconds)}
              </div>
      ...
      {editingNote && (
        ...
        >
          <div
            className="card p-5 w-full max-w-md space-y-3 animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-white font-display font-semibold">
              Note:{" "}
              <span className="text-brand-400">
                {nodes.find((n) => n.id === editingNote)?.label}
              </span>
            </h3>
            <textarea
              value={noteText}
              ...
              className="input !rounded-xl h-32 resize-none text-sm"
              autoFocus
            />
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">
                Your progress and notes will be permanently removed.
              </p>
            </div>
      ...
      {error && (
        <div className="mb-2 shrink-0 border rounded-xl px-4 py-2.5 text-sm bg-red-500/10 border-red-500/30 text-red-400 animate-view-in flex items-center justify-between gap-3">
          {error}
          <button
            onClick={fetchMap}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0 flex-1">
                      <button
                        ...
                        title={`Click: ${status} → ${NEXT_STATUS[status]}`}
                      >
                        <span
                          className={`text-sm transition-transform duration-200 hover:scale-125 inline-block ${
                            status === "known"
                              ? "text-emerald-700 dark:text-emerald-300"
                              : status === "partial"
                                ? "text-amber-700 dark:text-amber-300"
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span
                        className={`text-[11px] px-2.5 py-0.5 rounded-full capitalize font-semibold ${sc.badge}`}
                      >
                        {status}
                      </span>
                      {node.depth != null && (
                  ...
                  <div className="flex items-center gap-1.5 mt-3 flex-wrap">
                    {STATUSES.map((s) => (
                      <button
                        ...
                        className={`px-3 py-1 text-xs rounded-lg border transition-all capitalize font-medium ${
                          status === s
                            ? STATUS_COLORS[s].active
                            : s === "partial"
                              ? "bg-amber-50 text-amber-800 border-amber-500 dark:bg-transparent dark:text-amber-300 dark:border-amber-600"
                              : s === "known"
                                ? "bg-emerald-50 text-emerald-800 border-emerald-500 dark:bg-transparent dark:text-emerald-300 dark:border-emerald-600"
                                : "bg-slate-100 text-slate-800 border-slate-500 dark:bg-transparent dark:text-slate-300 dark:border-slate-600"
                        }`}
                      >
                        {s}
                      </button>
                    ...
                    <button
                      onClick={() => openNoteEditor(node.id)}
                      className={`px-2 py-0.5 text-[12px] rounded-lg border transition-all font-medium ${nodeNote ? "border-amber-500/30 text-amber-300 bg-amber-500/5" : "border-slate-700 text-slate-400 hover:border-slate-300  hover:text-slate-300"}`}
                    >
                      {nodeNote ? "✎ Note" : "+ Note"}
                    </button>
                    <button
