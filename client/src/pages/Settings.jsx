...
const Settings = () => {
  ...
  const handlePrefChange = async (key, value) => {
    ...
    try {
      ...
      toast("Failed to save preference", { type: "error" });
    } finally {
  ...
  const handleDonate = async (tierId) => {
    ...
    try {
      ...
      toast(err.response?.data?.message || "Donation failed", {
        type: err.response?.data?.configured === false ? "info" : "error",
      });
    } finally {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        ...
        toast("Thanks for sharing!", { type: "success" });
        return;
      } catch (e) {
  ...
  const handleCopyLink = () => {
    navigator.clipboard
      ...
      .then(() => {
        setCopied(true);
        toast("Link copied!", { type: "success", duration: 1500 });
  ...
  const Toggle = ({ on, onToggle, label, desc }) => (
    <div className="flex items-center justify-between p-4 rounded-xl border border-surface-border">
      ...
      >
        <span
          className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-md transition-transform duration-300 ${on ? "left-[calc(100%-22px)]" : "left-0.5"}`}
        />
      </button>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10 animate-slide-up">
      ...
      <h1 className="text-xl sm:text-2xl font-display font-bold text-white mb-6">
        Settings
      </h1>
        ...
        <div className="flex-1 min-w-0">
          ...
          {activeSection === "profile" && (
            <form
              ...
              className="card p-5 sm:p-6 space-y-5 animate-view-in"
            >
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ROLES.map((r) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-xs font-medium ${role === r.id ? "text-white" : "text-slate-400"}`}
                      >
                        {r.label}
                      </span>
                    </button>
            ...
            <div className="space-y-4 animate-view-in">
              <div className="card p-5 sm:p-6 space-y-4">
                <div className="pb-3 border-b border-surface-border flex items-center justify-between">
                  ...
                  {prefsSaving && (
                    <span className="text-[10px] text-brand-400 animate-pulse">
                      Saving...
                    </span>
                  )}
                </div>
                ...
                <div className="p-4 rounded-xl border border-surface-border">
                  ...
                  <p className="text-xs text-slate-500 mb-3">
                    Choose which view opens by default
                  </p>
                  <div className="flex gap-2">
                    {VIEW_OPTIONS.map((v) => (
                      <button
                        key={v.id}
                        onClick={() => handlePrefChange("defaultView", v.id)}
                ...
                <Toggle
                  on={prefs.autoSave}
                  onToggle={() => handlePrefChange("autoSave", !prefs.autoSave)}
                  label="Auto-save Progress"
                  desc="Automatically save when you change a node's status"
                />

                <Toggle
                  on={prefs.showDescriptions}
                  onToggle={() =>
                    handlePrefChange(
                      "showDescriptions",
                  ...
                  desc="Display concept descriptions in list and tree views"
                />

                <Toggle
                  on={prefs.compactMode}
                  onToggle={() =>
                    handlePrefChange("compactMode", !prefs.compactMode)
                  }
                  label="Compact Mode"
              ...
              <div className="card p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {[
                      ...
                      <div key={key} className="flex items-center gap-2">
                        <kbd className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-400 font-mono text-[10px]">
                          {key}
                        </kbd>
                ...
                <div className="p-4 rounded-xl border border-surface-border">
                  ...
                  <p className="text-xs text-slate-500 mb-3">
                    Export or clear your local cached data
                  </p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        localStorage.removeItem("nbs_theme");
                        localStorage.removeItem("nbs_recent_topics");
                        toast("Cache cleared — refresh to see defaults", {
                          type: "success",
                        });
                      }}
          ...
          {activeSection === "support" && (
            <div className="space-y-4 animate-view-in">
              <div className="card p-5 sm:p-6 relative overflow-hidden">
                ...
                <div className="relative">
                  <div className="flex items-center gap-3 mb-4 pb-4 border-b border-surface-border">
                    ...
                    <div>
                      <h2 className="text-white font-display font-semibold">
                        Support {APP_CONFIG.name}
                      </h2>
                      <p className="text-xs text-slate-500 mt-0.5">
                        Help us keep building and improving this tool
                      </p>
                    </div>
                  </div>

                  <p className="text-sm text-slate-400 mb-5 leading-relaxed">
                    {APP_CONFIG.name} is free and open to all. If it's been
                    helpful to you, consider supporting the development. Every
                    contribution goes directly towards server costs, AI API
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                    {tiers.map((tier) => (
                      <button
                        ...
                        disabled={donateLoading === tier.id}
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      disabled={donateLoading === "custom" || !customDonation}
              ...
              <div className="card p-5 space-y-3">
                <h3 className="text-white text-sm font-display font-semibold">
                  Other ways to support
                </h3>
                ...
                >
                  ...
                  <svg
                    className="w-4 h-4 text-slate-600 group-hover:text-slate-400 shrink-0 transition-colors"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
              ...
              {shareOpen && (
                ...
                >
                  <div
                    className="card p-5 sm:p-6 w-full max-w-sm space-y-4 animate-scale-in"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div className="flex items-center justify-between">
                      <h3 className="text-white font-display font-semibold">
                        Share {APP_CONFIG.name}
                      </h3>
                      <button
                        onClick={() => setShareOpen(false)}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <a
                        href={`mailto:?subject=${encodeURIComponent(`Check out ${APP_CONFIG.name}`)}&body=${encodeURIComponent(`${shareText}\n\n${shareUrl}`)}`}
