...
const SUGGESTED_TOPICS = [
  "Machine Learning", "React", "Data Structures", "System Design",
  "Python", "Docker", "Kubernetes", "GraphQL", "SQL", "Cybersecurity",
  "Blockchain", "Computer Networks", "Operating Systems", "Web Security",
  "TypeScript", "Next.js", "Redis", "PostgreSQL", "Rust", "Go",
  "Linear Algebra", "Statistics", "Deep Learning", "NLP", "Computer Vision",
];
...
let _recents = [];
function getRecents() { return _recents; }
function addRecent(topic) {
  _recents = [topic, ..._recents.filter((t) => t.toLowerCase() !== topic.toLowerCase())].slice(0, 8);
}
...
const Dashboard = () => {
  ...
  const filteredMaps = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter((m) => m.topicMap?.topic?.toLowerCase().includes(q));
    }
    result = [...result].sort((a, b) => {
      const pctA = a.stats?.total ? (a.stats.known + a.stats.partial * 0.5) / a.stats.total : 0;
      const pctB = b.stats?.total ? (b.stats.known + b.stats.partial * 0.5) / b.stats.total : 0;
      switch (sort) {
        case "oldest":  return new Date(a.updatedAt) - new Date(b.updatedAt);
        case "most":    return pctB - pctA;
        case "least":   return pctA - pctB;
  ...
  const globalStats = useMemo(() => {
    let totalConcepts = 0, totalKnown = 0, totalPartial = 0, totalUnknown = 0;
    maps.forEach((m) => {
    ...
    const mastery = totalConcepts > 0 ? Math.round(((totalKnown + totalPartial * 0.5) / totalConcepts) * 100) : 0;
    return { maps: maps.length, concepts: totalConcepts, known: totalKnown, partial: totalPartial, unknown: totalUnknown, mastery };
  }, [maps]);

  // Most recently accessed map (for "jump back in")
  const recentMap = useMemo(() => {
    if (!maps.length) return null;
    return [...maps].sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt))[0];
  }, [maps]);

  const recentTopics = getRecents();
  const suggestions = topic.trim()
    ? SUGGESTED_TOPICS.filter((s) => s.toLowerCase().includes(topic.trim().toLowerCase()) && s.toLowerCase() !== topic.trim().toLowerCase()).slice(0, 5)
    : [...recentTopics.slice(0, 3), ...SUGGESTED_TOPICS.filter((s) => !recentTopics.includes(s)).slice(0, 5)];
  ...
  const greetingEmoji = new Date().getHours() < 12 ? "☀️" : new Date().getHours() < 17 ? "🌤️" : "🌙";
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      <div className="mb-8 sm:mb-10 animate-slide-up">
        <div className="flex items-center gap-3 mb-1">
          <h1 className="text-2xl sm:text-4xl capitalize tracking-tight">
            <span className="font-serif italic font-medium text-white">{greeting},</span>{" "}
            <span className="font-display font-bold text-white">{user?.name?.split(" ")[0]}</span>
          </h1>
          <span className="text-2xl sm:text-3xl animate-float">{greetingEmoji}</span>
        </div>
        <p className="text-slate-400 text-sm sm:text-base font-body">{APP_CONFIG.tagline}</p>
      </div>
      ...
      {!loading && maps.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 animate-slide-up opacity-0 stagger-1">
          {[
            { label: "Maps",     value: globalStats.maps,       icon: "🗺️", color: "text-brand-400",   sub: "topics mapped" },
            { label: "Concepts", value: globalStats.concepts,   icon: "🧩", color: "text-violet-400",  sub: `${globalStats.unknown} gaps left` },
            { label: "Mastered", value: globalStats.known,      icon: "✓",  color: "text-emerald-400", sub: `${globalStats.partial} partial` },
            { label: "Mastery",  value: `${globalStats.mastery}%`, icon: "📈", color: "text-amber-400", sub: "overall progress" },
          ].map((s) => (
            <div key={s.label} className="card px-4 py-3.5 flex items-center gap-3 group hover:border-slate-600 transition-colors">
              <span className="text-lg">{s.icon}</span>
              <div>
                <p className={`text-lg font-display font-bold ${s.color}`}>{s.value}</p>
                <p className="text-[10px] text-slate-500 font-medium leading-tight">{s.sub}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {!loading && recentMap && (
        <div className="mb-5 animate-slide-up opacity-0 stagger-2">
          <p className="text-xs text-slate-500 uppercase tracking-wider font-medium mb-2">Jump back in</p>
          <button
          ...
          >
            <ProgressRing known={recentMap.stats?.known || 0} partial={recentMap.stats?.partial || 0} total={recentMap.stats?.total || 0} size={44} />
            <div className="flex-1 min-w-0">
              <p className="text-white font-display font-semibold capitalize text-sm group-hover:text-brand-400 transition-colors truncate">{recentMap.topicMap?.topic}</p>
            ...
            <svg className="w-4 h-4 text-slate-600 group-hover:text-brand-400 transition-colors shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="card p-5 sm:p-6 mb-6 sm:mb-8 animate-slide-up opacity-0 stagger-2 relative isolate z-20">
        ...
        <div className="relative">
          <h2 className="text-white font-display font-semibold mb-1">Generate a Knowledge Map</h2>
          <p className="text-slate-500 text-xs mb-3">Enter any topic and our AI will map out everything you need to learn</p>

          <Alert type="error" message={error} />

          <form onSubmit={handleGenerate} className="flex flex-col sm:flex-row gap-3 mt-2">
              ...
              {showSuggestions && suggestions.length > 0 && !generating && (
                <div className="absolute top-full left-0 right-0 mt-1 rounded-xl border border-surface-border bg-surface-card p-1.5 z-50 max-h-52 overflow-y-auto animate-scale-in origin-top shadow-2xl shadow-black/40">
                  {recentTopics.length > 0 && !topic.trim() && (
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider px-3 py-1 font-medium">Recent</p>
                  )}
                  {suggestions.map((s, i) => (
                    <button key={i} type="button"
                      onMouseDown={(e) => { e.preventDefault(); setTopic(s); setShowSuggestions(false); }}
                      ...
                      {recentTopics.includes(s) && !topic.trim() ? (
                        <svg className="w-3 h-3 text-slate-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      ) : (
                        <svg className="w-3 h-3 text-slate-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                      )}
                      {s}
                    </button>
                  ))}
            ...
            >
              {generating ? (
                <>
                  <Spinner size="sm" />
                  <span className="animate-pulse">{topic.trim() ? `Mapping "${topic.trim()}"...` : "Generating..."}</span>
                </>
              ) : (
                <>
          ...
          {!topic.trim() && !generating && (
            <div className={`flex flex-wrap gap-1.5 mt-3 transition-opacity duration-200 ${showSuggestions ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
              {SUGGESTED_TOPICS.slice(0, 10).map((t) => (
                <button key={t} type="button" onClick={() => setTopic(t)}
                  className="px-2.5 py-1 text-[11px] rounded-lg border border-surface-border text-slate-500 hover:text-white hover:border-brand-500/40 hover:bg-brand-500/5 transition-all duration-200 font-medium">
                  {t}
                </button>
              ))}
            </div>
      ...
      <div className="mb-4 space-y-3 animate-slide-up opacity-0 stagger-3 relative z-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <h2 className="text-white text-lg font-display font-semibold">
            Your Maps
            <span className="text-slate-500 font-body font-normal text-sm ml-2">({filteredMaps.length})</span>
          </h2>
          ...
          {maps.length > 0 && (
            <div className="relative">
              ...
              <input
                ref={searchRef}
                type="text" placeholder="Search maps…" value={search}
      ...
      <div>
        {loading ? (
            ...
            <EmptyState
              icon={
                <div className="w-20 h-20 mx-auto rounded-2xl bg-gradient-to-br from-brand-500/15 to-violet-500/10 border border-brand-500/20 flex items-center justify-center animate-float">
                  <svg className="w-10 h-10 text-brand-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
          ...
          <div className="text-center py-12 space-y-2 animate-fade-in">
            ...
            <button onClick={() => setSearch("")} className="text-xs text-brand-400 hover:text-brand-300 transition-colors">Clear search</button>
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredMaps.map((map, i) => {
              const pct = map.stats?.total ? Math.round(((map.stats.known + map.stats.partial * 0.5) / map.stats.total) * 100) : 0;
                ...
                >
                  {isRecent && (
                    <div className="absolute top-3 left-3">
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-brand-500/15 text-brand-400 border border-brand-500/20 font-medium">Recent</span>
                    </div>
                  )}

                  <button
                    onClick={(e) => { e.stopPropagation(); setDeleteTarget(map.topicMap._id); }}
                  ...
                  <div className={`flex items-start gap-3.5 ${isRecent ? "mt-5" : ""}`}>
                    <ProgressRing known={map.stats?.known || 0} partial={map.stats?.partial || 0} total={map.stats?.total || 0} size={50} />
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-3 text-xs text-slate-500">
                        <span className="flex items-center gap-1" title="Known">
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />{map.stats?.known || 0}
                        </span>
                        <span className="flex items-center gap-1" title="Partial">
                          <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />{map.stats?.partial || 0}
                        </span>
                        <span className="flex items-center gap-1" title="Unknown">
                          <span className="w-1.5 h-1.5 rounded-full bg-slate-600" />{map.stats?.unknown || 0}
                        </span>
                  ...
                  <div className="mt-3 h-1 rounded-full bg-slate-800 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        background: pct === 100 ? "#10b981" : pct > 50 ? "linear-gradient(90deg,#10b981,#f59e0b)" : "#3b82f6",
                      }}
                    />
                  </div>

                  <div className="flex items-center justify-between mt-2">
                    <span className="text-[11px] text-slate-500 font-mono">{relativeTime(map.updatedAt)}</span>
                    {isMastered
                      ? <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 font-medium border border-emerald-500/20">Mastered</span>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setDeleteTarget(null)}>
          <div className="card p-6 w-full max-w-sm space-y-4 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-center">
              <h3 className="text-white font-display font-semibold">Delete this map?</h3>
              <p className="text-sm text-slate-400 mt-1">Your progress and notes for this map will be permanently removed.</p>
            </div>
